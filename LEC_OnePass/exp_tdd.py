...
class exp:
    def __init__(self, cuda, model, epochs, learning_rate, train_dataloader, valid_dataloader, test_dataloader, dataset, best_PATH, load_model_path, dpn, model_name = None):
        self.cuda = cuda
        self.model = model
        ...
        if eval_data == "MATRES":
            try:  
                if self.dpn:
                    ...
                    F1 = f1_score(tri_gold, tri_pred, average='micro')
                    print("  macro F1: {0:.3f}".format(macro_f1))
                    print("  micro F1: {0:.3f}".format(F1))
