...
def count_parameters(model_old):
    return sum(p.numel() for p in model_old.parameters() if p.requires_grad)
...
params = {'transformers_model': 'google/bigbird-roberta-base',
          ...
          'debug': 0,
          'rst_file_name': "1116-lr3e-6-b5-gpu7-loss0-dataHiEve.rst",    # exp-2778
         }
...
model_old = transformers_mlp_cons(params)
model_old.to(cuda)
#model.zero_grad()
print("# of parameters:", count_parameters(model_old))
...
class MyWebService(object):
    ...
    def annotate(self):
        ...
        if hasJSON:
            ...
            mem_exp = exp(cuda, model_old, params['epochs'], params['learning_rate'], None, None, test_dataloader, params['dataset'], best_PATH, None, model_name)
            ...
            for i in range(1, event_num+1):
                for j in range(i+1, event_num+1):
                    pairs.append([i, j])
            for [pair_count, pred] in pc_in_doc:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--port", default=6004, type=int, required=False,
