...
class exp:
    def __init__(self, cuda, model, epochs, learning_rate, train_dataloader, valid_dataloader, test_dataloader, dataset, best_PATH, load_model_path, model_name = None):
        self.cuda = cuda
        self.model = model
    ...
    def evaluate(self, eval_data, test = False, predict = False):
        ...
        for batch in dataloader:
            ...
            y_predict = torch.max(logits[:, 0:4], 1).indices.cpu().numpy()
            ...
            y_logits = np.append(y_logits, logits[:, 0:4].cpu().numpy(), 0) # for prediction result output
